...
class Command {
	...
	protected static function parse($task)
	{
		...
		// on ittasks by separating the task and method with a single colon.
	...
	public static function resolve($bundle, $task)
	{
		...
		// injected into ittasks for more flexible testability.
		...
		if (file_exists($path = Bundle::path($bundle).'ittasks/'.$task.EXT))
		{
			require_once $path;
